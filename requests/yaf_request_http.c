...
yaf_request_t *yaf_request_http_instance(yaf_request_t *this_ptr, zend_string *request_uri, zend_string *base_uri) /* {{{ */ {
	...
	yaf_write_property(yaf_request_http_ce, this_ptr, YAF_REQUEST_PROPERTY_NAME_METHOD, &method);
	...
	if (settled_uri) {
		...
		yaf_write_property(yaf_request_http_ce, this_ptr, YAF_REQUEST_PROPERTY_NAME_URI, &rv);
		yaf_request_set_base_uri(this_ptr, base_uri, settled_uri);
		zend_string_release(settled_uri);
	}

	array_init(&params);
	yaf_write_property(yaf_request_http_ce, this_ptr, YAF_REQUEST_PROPERTY_NAME_PARAMS, &params);
